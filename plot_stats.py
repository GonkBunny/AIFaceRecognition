...
def plot_evolution(info, prefix="AL"):
    plt.figure()
    plt.title("Algorithms score at each iteration")
    plt.xlabel("Iteration")
    plt.ylabel("Score [0; 1]")
    for i in range(2, len(info.columns)):
        plt.plot(info.columns[0], info.columns[i], data=info)
    plt.legend(fontsize='xx-small')
    plt.tight_layout()
    plt.savefig("plots/%s_%s" % (prefix, "Algorithms score at each iteration"))
    plt.show()


def plot_evolution_trend(info, prefix="AL"):
    plt.figure()
    ...
    plt.legend(fontsize='xx-small')
    plt.tight_layout()
    plt.savefig("plots/%s_%s" % (prefix, "Algorithms score at each iteration approximated by a trend line"))
    plt.show()


def plot_evolution_speed_trend(info, prefix="AL"):
    plt.figure()
    ...
    plt.legend(fontsize='xx-small')
    plt.tight_layout()
    plt.savefig("plots/%s_%s" % (prefix, "Derivative of algorithms score at each iteration approximation"))
    plt.show()


def plot_best_iteration(info, prefix="AL"):
    plt.figure()
    ...
    plt.legend(fontsize='xx-small')
    plt.tight_layout()
    plt.savefig("plots/%s_%s" % (prefix, "Algorithms score at their best iteration"))
    plt.show()
...
def main():
    ...
    plot_evolution(al, prefix="AL")
    plot_evolution_trend(al, prefix="AL")
    plot_evolution_speed_trend(al, prefix="AL")
    plot_best_iteration(al, prefix="AL")

    pl = pandas.read_csv("passive_learning_iterations.csv")
    pl.dropna(inplace=True)
    plot_evolution(pl, prefix="PL")
    plot_evolution_trend(pl, prefix="PL")
    plot_evolution_speed_trend(pl, prefix="PL")
    plot_best_iteration(pl, prefix="PL")
